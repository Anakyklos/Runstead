...
## Maintainer review fix round 3 (PR #105 head, completed)

- [x] R07 [US2] Preserve semantic section order in the render with single-charge accounting: `renderCompiled` writes each section's pinned lines in order and admits a degradable line only when it fits IN ADDITION to all future pinned bytes (futurePinned reservation); every authoritative section/detail stays before the NON-AUTHORITATIVE marker and only non-authoritative notes follow it. Output always <= MaxContextBytes; exact-fit boundary test retained. New test: `TestNonAuthoritativeBoundaryInRender` (authoritative degradable material before the marker, note after, using the LAST marker occurrence because the authority preamble also mentions the boundary).
