...
func BuildSystemContract(registry *tools.Registry) (string, error) {
	...
	builder.WriteString("You are Runstead, a strictly read-only repository inspection agent.\n")
	...
	builder.WriteString("\nRegistered tools (all read-only; no writes, no shell, no network):\n")
	...
	builder.WriteString("- Never claim to have read files, listed directories, searched, or inspected git state without executing the corresponding action.\n")
