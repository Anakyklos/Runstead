...
const (
	...
	familyMessagesPath    = "v1/messages"
	maxTokensStringBounds = 9 // 999,999,999; a sane operator ceiling, endpoint-enforced beyond that
)

// Protocol option keys this adapter consumes from provider.Resolved.Options.
// These are the strictly necessary NON-SECRET Messages-style wire parameters
// that provider.Request does not carry (#88): the generation limit and the
// versioning/header semantics. Config.Options already represents them; the
// #79 contract extension propagates a validated defensive copy through
// provider.Resolved. Unknown option keys are refused at New: silently ignoring
// operator configuration would make the protected single-attempt baseline
// unprovable.
const (
	optionMaxTokens        = "max_tokens"
	optionAnthropicVersion = "anthropic_version"
	headerAnthropicVersion = "anthropic-version"
	headerAPIKey           = "x-api-key"
	headerClientRequestID  = "X-Runstead-Client-Request-ID"
	knownOptionKeys        = optionMaxTokens + "," + optionAnthropicVersion
)
...
// anthropic_compatible family. One Complete call performs exactly one physical
...
// New builds an Anthropic-compatible protocol adapter around a resolved
...
func New(resolved provider.Resolved, resolver SecretResolver, options Options) (*Client, error) {
	...
	if resolved.ProtocolFamily != provider.FamilyAnthropicCompatible {
		return nil, configRefusedError(fmt.Errorf("protocol family %q is not %q", string(resolved.ProtocolFamily), string(provider.FamilyAnthropicCompatible)))
	}
	...
	if _, err := messagesURL(resolved.BaseURL); err != nil {
		return nil, configRefusedError(fmt.Errorf("invalid base URL for messages: %w", err))
	}
	// The Messages-style wire requires a generation limit and a versioned
	// header that provider.Request cannot carry. Both must be present in the
	// validated non-secret options; anything unknown, missing or invalid
	// refuses the adapter before any dispatch. No silent defaults exist.
	maxTokens, version, err := resolveProtocolOptions(resolved.Options)
	if err != nil {
		return nil, configRefusedError(err)
	}
...
func (c *Client) Complete(ctx context.Context, request provider.Request) (provider.Response, error) {
	...
	endpointURL, err := messagesURL(c.resolved.BaseURL)
	if err != nil {
	...
	payload, err := encodeMessagesRequest(c.resolved.Model, c.maxTokens, request.Prompt)
	if err != nil {
		return notSent(ErrorConfigRefused, err)
	}
	...
	httpReq.Header.Set(headerAnthropicVersion, c.anthropicVersion)
	if apiKey != "" {
		httpReq.Header.Set(headerAPIKey, apiKey)
	}
	if request.ClientRequestID != "" {
		httpReq.Header.Set(headerClientRequestID, sanitizeHeaderToken(request.ClientRequestID))
	}
	started := c.now()
...
func (c *Client) responseMetadata(response *http.Response, duration time.Duration, endpointURL string) provider.ResponseMetadata {
	return provider.ResponseMetadata{
		...
		RequestID:  hashOpaque(response.Header.Get("request-id")),
		Duration:   duration,
...
// resolveProtocolOptions validates the closed option vocabulary this adapter
// implements and returns the parsed generation limit and version. Any
// unsupported key, missing required key or invalid value refuses construction
// (zero requests); there are no silent defaults and no generic option catalog.
func resolveProtocolOptions(options map[string]string) (int, string, error) {
	for key := range options {
		if key != optionMaxTokens && key != optionAnthropicVersion {
			return 0, "", fmt.Errorf("unsupported protocol option %q: this adapter implements only %s", key, knownOptionKeys)
		}
	}
	rawMaxTokens, ok := options[optionMaxTokens]
	if !ok {
		return 0, "", fmt.Errorf("protocol option %q is required by the Messages-style wire (generation limit)", optionMaxTokens)
	}
	maxTokens, err := strconv.Atoi(strings.TrimSpace(rawMaxTokens))
	if err != nil || maxTokens <= 0 {
		return 0, "", fmt.Errorf("protocol option %q must be a positive integer, got %q", optionMaxTokens, rawMaxTokens)
	}
	if len(strings.TrimSpace(rawMaxTokens)) > maxTokensStringBounds {
		return 0, "", fmt.Errorf("protocol option %q is unreasonably large", optionMaxTokens)
	}
	version, ok := options[optionAnthropicVersion]
	if !ok {
		return 0, "", fmt.Errorf("protocol option %q is required by the Messages-style wire (versioned header semantics)", optionAnthropicVersion)
	}
	version = strings.TrimSpace(version)
	if _, err := time.Parse("2006-01-02", version); err != nil {
		return 0, "", fmt.Errorf("protocol option %q must be a YYYY-MM-DD version date, got %q", optionAnthropicVersion, version)
	}
	return maxTokens, version, nil
}
...
// messagesURL derives the family endpoint from the validated base URL. The
// Messages family defines POST /v1/messages as its canonical path, so the
// configured base URL is the API root and the family path is appended,
// preserving any operator prefix (for example /anthropic behind a gateway).
func messagesURL(rawBaseURL string) (string, error) {
	...
	baseURL.Path = strings.TrimRight(baseURL.Path, "/") + "/" + familyMessagesPath
	baseURL.RawPath = ""
