...
// TestAgentLoopNeverSeesAnthropicWireTypes is the structural proof for the
// amplification invariant "the agent loop cannot know Anthropic": the packages
// above the provider boundary (agent, governor, tools, verifier) must not
// reference Anthropic wire concepts, vendor endpoints or this adapter. The
// only permitted match is an environment-variable redaction fixture in tool
// tests, which proves secrets are REDACTED, not consumed.
func TestAgentLoopNeverSeesAnthropicWireTypes(t *testing.T) {
	inspector := &anthropicSourceInspector{t: t}
	for _, dir := range []string{
		...
		inspector.assertNoAnthropicWireConcepts(dir)
	}
}

type anthropicSourceInspector struct {
	t *testing.T
}

func (s *anthropicSourceInspector) assertNoAnthropicWireConcepts(dir string) {
	s.t.Helper()
	...
	for _, entry := range entries {
		...
		if entry.IsDir() {
			s.assertNoAnthropicWireConcepts(path)
			continue
		}
		...
		for _, forbidden := range []string{"anthropic", "anthropiccompat", "v1/messages", "anthropic-version", "x-api-key", "max_tokens", "content_block", "tool_use", "stop_details"} {
			if strings.Contains(lower, forbidden) {
				s.t.Errorf("%s references Anthropic wire concept %q across the provider boundary", path, forbidden)
			}
		}
	}
}
...
func (p *amplificationProof) client(t *testing.T, baseURL string) *anthropiccompat.Client {
	t.Helper()
...
func TestAmplificationErrorScenariosNeverFireSecondRequest(t *testing.T) {
	scenarios := map[string]http.HandlerFunc{
		...
		"refused turn": func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(http.StatusOK)
...
// the #87 review and carried into #88: a retry/fan-out-capable dispatch stack
// must not be able to pose as SafeRouteSafety. The constructor accepts no
// http.Client and no RoundTripper at all, so an amplifying stack can never be
// installed where the adapter could not observe or account for it; this test
// pins that surface by compile-time shape (Options has no transport fields)
// plus a runtime probe showing the adapter's client keeps its own pinned knobs.
func TestAmplifyingTransportStackCannotBeInjected(t *testing.T) {
	...
	options := anthropiccompat.Options{}
	optionsType := reflect.TypeOf(options)
	if optionsType.NumField() != 1 || optionsType.Field(0).Name != "Now" {
		t.Fatalf("anthropiccompat.Options must expose only Now; found %d fields with transport injection surfaces", optionsType.NumField())
	}
...
func TestAmbiguousTransportErrorIsConservativeNotNotSent(t *testing.T) {
	...
	if adapterErr.Kind != anthropiccompat.ErrorTransport {
...
func TestDeliveryStateProgressionPinsConservativeEvidence(t *testing.T) {
	...
	t.Run("upstream denial body fully read is completed", func(t *testing.T) {
		recorder := newRequestRecorder(t, func(w http.ResponseWriter, r *http.Request) {
			...
			_, _ = w.Write([]byte(`{"type":"error","error":{"message":"nope"}}`))
		})
		client, _ := newTestClient(t, func(c *provider.Config) { c.BaseURL = recorder.server.URL }, nil, recorder)
		response, err := client.Complete(context.Background(), provider.Request{Prompt: "p"})
		if err == nil {
