...
// Integration-boundary validation: the real Anthropic-compatible adapter
// (issue #88) driven through the governor's public Execute interface against a
...
//     adapter -> Governor.Execute -> real server) resolves authentication
//     only at dispatch, lands it on the wire as the x-api-key header, and
//     never leaks the secret into identity, results or evidence.
...
type anthropicCompatCounter struct {
	mu       sync.Mutex
	requests int
	lastPath string
	lastID   string
...
func (c *anthropicCompatCounter) server() *httptest.Server {
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("request-id", "req_integration_1")
		fmt.Fprint(w, `{"id":"msg_integration_1","type":"message","role":"assistant","content":[{"type":"text","text":"hello from upstream"}],"model":"model-a","stop_reason":"end_turn","stop_sequence":null,"usage":{"input_tokens":2,"output_tokens":3}}`)
	}))
}

func (c *anthropicCompatCounter) count() int {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.requests
}

func (c *anthropicCompatCounter) authorization() string {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.lastAuth
}

func (c *anthropicCompatCounter) snapshot() (string, string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.lastPath, c.lastID
}

func newAnthropicCompatAdapter(t *testing.T, baseURL string) *anthropiccompat.Client {
	t.Helper()
	client, err := anthropiccompat.New(provider.Resolved{
	...
	}, nil, anthropiccompat.Options{})
	if err != nil {
		t.Fatalf("anthropiccompat.New() error = %v", err)
	}
	return client
}
...
func TestAnthropicCompatAdapterServesGovernorExecuteWithExactlyOnePhysicalRequest(t *testing.T) {
	...
	if path != "/v1/messages" {
		t.Fatalf("wire path = %q, want /v1/messages", path)
	}
	if id != "request-1" {
...
func TestAnthropicCompatAdapterRejectedFailClosedOnRouteMismatchWithZeroRequests(t *testing.T) {
	counter := &anthropicCompatCounter{}
	server := counter.server()
	defer server.Close()
...
// TestAnthropicCompatAdapterFullPublicPathConfigToWireWithAuthSecret drives the
...
// auth-reference validation) -> anthropiccompat.New with a SecretResolver ->
// Governor.Execute against a real server. It proves the secret reference is
// resolved only at dispatch time, lands on the wire as the x-api-key header,
// and never appears in the config identity or in the execution
// result/evidence.
func TestAnthropicCompatAdapterFullPublicPathConfigToWireWithAuthSecret(t *testing.T) {
	counter := &anthropicCompatCounter{}
	server := counter.server()
	defer server.Close()
	...
	if resolved.Options["max_tokens"] != "1024" || resolved.Options["anthropic_version"] != "2023-06-01" {
		t.Fatalf("resolved options = %v, want the validated non-secret protocol options", resolved.Options)
	}

	client, err := anthropiccompat.New(*resolved, func(_ context.Context, reference provider.SecretRef) (string, error) {
	...
	if got := counter.authorization(); got != secretValue {
		t.Fatalf("wire x-api-key = %q, want the resolved secret material", got)
	}
...
// TestAnthropicCompatAdapterRateLimitCrossesGovernorBoundaryWithSingleRequest
...
// classifier reads anthropiccompat.Error.Kind/DeliveryState via errors.As (no
...
func TestAnthropicCompatAdapterRateLimitCrossesGovernorBoundaryWithSingleRequest(t *testing.T) {
	counter := &anthropicCompatCounter{}
	...
	classifier := func(response provider.Response, err error) policy.Outcome {
		var upstreamErr *anthropiccompat.Error
		if errors.As(err, &upstreamErr) {
			seenRetryAfter = upstreamErr.RetryAfter
			switch upstreamErr.Kind {
			case anthropiccompat.ErrorRateCapacity:
...
func TestAnthropicCompatAdapterTimeoutCrossesGovernorBoundaryConservatively(t *testing.T) {
	counter := &anthropicCompatCounter{}
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		counter.mu.Lock()
		counter.requests++
