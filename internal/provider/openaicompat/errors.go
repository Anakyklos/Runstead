// Package openaicompat implements the provider.Client contract for the
// openai_compatible protocol family (#87). It is a protocol adapter, not an
// integration with any specific vendor: the official OpenAI service, local
// gateways and third-party endpoints are all just implementations of the same
// minimal Chat Completions subset this adapter speaks.
//
...
const (
	...
	// unsafe route safety, model mismatch, unusable endpoint. Zero requests.
	...
	// ErrorEmptyResponse is an empty body or an empty/incompatible content.
...
func (e *Error) Error() string {
	if e == nil {
		return "openai_compatible error"
	}
	if e.StatusCode > 0 {
		return fmt.Sprintf("openai_compatible %s (status %d)", e.Kind, e.StatusCode)
	}
	return fmt.Sprintf("openai_compatible %s", e.Kind)
}
