...
func TestFirstTokenLatencyUsesInjectedClock(t *testing.T) {
	...
	client, err := openaicompat.New(resolved, nil, openaicompat.Options{Now: clock})
	if err != nil {
	...
	// The request is provably inside the handler, so `started` was already
	// sampled with the clock at T0.
	<-entered
