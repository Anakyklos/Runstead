// Package googlecompat implements the provider.Client contract for the
// google_compatible protocol family (#89). It is a protocol adapter, not an
// integration with any specific vendor: the official Google Gemini service,
// local gateways and third-party endpoints are all just implementations of the
// same minimal generateContent-style subset this adapter speaks. Provider
// identity stays completely separate from the protocol family; two different
// provider IDs of this family are served by the same adapter path with no
...
const (
	...
	// unsafe route safety, model mismatch, unusable endpoint, unsupported
	// protocol options. Zero requests.
	...
	// cannot prove and consume: function-call parts and function-call finish
	// reasons when native tools are not enabled, plus non-text parts. These
	// are NEVER interpreted as text or as Runstead task truth.
	ErrorUnsupportedResponseFormat ErrorKind = "unsupported_response_format"
	// ErrorIncompleteCompletion covers generateContent finish reasons that
	// prove the generation was truncated (MAX_TOKENS). The transport
	// delivered the body; the completion is not a complete
	// runstead.protocol.v1 turn.
	ErrorIncompleteCompletion ErrorKind = "incomplete_completion"
	// ErrorRefusal covers a provable refusal/safety state (a blocked prompt
	// through promptFeedback.blockReason or a safety/refusal finishReason),
	// classified from typed structure, never from parsing free text.
...
func (e *Error) Error() string {
	if e == nil {
		return "google_compatible error"
	}
	if e.StatusCode > 0 {
		return fmt.Sprintf("google_compatible %s (status %d)", e.Kind, e.StatusCode)
	}
	return fmt.Sprintf("google_compatible %s", e.Kind)
}
...
//
// The generateContent family carries statuses whose recovery semantics differ
// from a generic 5xx and must stay conservative:
...
func httpError(metadata provider.ResponseMetadata, statusCode int) *Error {
	...
	case statusCode == http.StatusGatewayTimeout:
		kind = ErrorTimeout
	case statusCode == 529:
		// 529 is the "overloaded" status: the upstream is at capacity. It is a
		// quota/capacity signal, classified as such.
		kind = ErrorRateCapacity
