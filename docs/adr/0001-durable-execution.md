...
- the **`execution_id` space is Runstead-owned**: tool attempts and provider
  ...
  and no receipt identifier may be used as a Runstead row identity. Even when
  a provider execution has exactly one receipt, the two identifiers remain
  distinct and are stored in different entities;
...
| --- | --- | --- | --- |
| `not_sent` | the request never left Runstead; no effect occurred | `planned` / `prepared` | Retry is replay-safe within budgets. |
...
| `response_started` | the response stream started; the effect occurred | `running` / `observed` | No retry; complete the observation and classify the result. |
...
when the upstream may have been reached. Persisting delivery state must not
become a second, contradicting lifecycle for the same attempt.
...
6. Runstead identities (`execution_id`, `action_id`) and upstream receipt
   identities (`receipt_attempt_id`) live in different ID spaces and are
