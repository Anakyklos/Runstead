...
truncated request id, type, window (baseline/turn/post_turn/pre_turn) and
turn id; then status, mimeType, completion (`finished`/`failed`), errorText,
and cumulative SSE bytes. Headers, cookies, bodies and query strings are never
read or logged (structural allowlist in `lib/network.mjs`).

**Exhaustive per-turn accounting (no orphaned windows).** The between-turns
gap (after a turn's post-turn window closes and before the next turn
dispatches) is an explicit **`pre_turn[N]`** window attributed to the upcoming
turn N. The no-hidden-retry/fan-out verdict for turn N is computed over the
exhaustive scope `{ initial-baseline (turn 0) for N==1 } ∪ pre_turn[N] ∪
turn[N] ∪ post_turn[N]` via a single shared rule
(`exhaustiveModelEffectScope` in `lib/proofs.mjs`) used by BOTH the runtime
(`run_spike.mjs`) and the canonical rebuild (`rebuild_evidence.mjs`). Any
`model_effect_conversation` or `potential_model_effect` observed anywhere
between the session's initial baseline and the final post-turn close therefore
belongs to exactly one turn's verdict. An un-attributable (orphaned)
...
7. **Exhaustive per-turn accounting (no orphaned windows).** The between-turns
   window before a turn's dispatch is now an explicit `pre_turn[N]` epoch
   attributed to that turn (review #4954036871). The no-hidden-retry/fan-out
   verdict for each turn is computed with a single shared exhaustive rule
   (`exhaustiveModelEffectScope`) used by both `run_spike.mjs` and
   `lib/rebuild_evidence.mjs`, covering `baseline(t0) ∪ pre_turn[N] ∪
   turn[N] ∪ post_turn[N]`. Deterministic fixtures
   (`evidence/fail-closed-proofs.json`) prove a known or potential model effect
   in a pre-turn window both blocks the clean verdict and is not orphaned.
   An orphaned model-effect request is a **terminal fail-closed** via the
   single shared `assertNoOrphanedModelEffects`, invoked at turn 1, at turn 2,
   and by the rebuild; it throws before any clean/final verdict is emitted,
   so detection at the end of turn 2 can no longer be observed while the
   harness still concludes success (review #4954384690). The decision path is
   proven deterministically (section `orphaned_effect_fail_closed_terminal`).
   No model-effect request can escape every turn's verdict.
...
- `evidence/fail-closed-proofs.json` — deterministic zero-turn proofs
  ...
  exhaustive per-turn model-effect scope incl. pre-turn windows, and the
  decision-path proof that an orphaned model-effect request is terminal
  fail-closed via `assertNoOrphanedModelEffects`,
  section `orphaned_effect_fail_closed_terminal`),
...
| 12 | no hidden retry/fan-out observed | PROVEN (this run, exhaustive: every model-effect request belongs to exactly one turn's scope; unknown conversation-namespace POSTs or pre-turn model effects flip the verdict to blocked) |
