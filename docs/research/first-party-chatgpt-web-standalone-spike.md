...
(`exhaustiveModelEffectScope`/`hasOrphanedModelEffects` in `lib/proofs.mjs`)
used by BOTH the runtime (`run_spike.mjs`) and the canonical rebuild
(`rebuild_evidence.mjs`). Any `model_effect_conversation` or
`potential_model_effect` observed anywhere between the session's initial
baseline and the final post-turn close therefore belongs to exactly one turn's
verdict; an un-attributable (orphaned) model-effect request fails the rebuild
gate and the runtime fails closed (`orphaned_model_effect`). This closes the
review finding that a model effect in the between-turns baseline window could
escape both turn verdicts.
...
7. **Exhaustive per-turn accounting (no orphaned windows).** The between-turns
   ...
   (`exhaustiveModelEffectScope`/`hasOrphanedModelEffects`) used by both
   `run_spike.mjs` and `lib/rebuild_evidence.mjs`, covering
   `baseline(t0) ∪ pre_turn[N] ∪ turn[N] ∪ post_turn[N]`. Deterministic
   fixtures (`evidence/fail-closed-proofs.json`, section
   `exhaustive_model_effect_scope`) prove a known or potential model effect in
   a pre-turn window both blocks the clean verdict and is not orphaned; the
   runtime and the canonical rebuild each fail closed on an orphaned
   model-effect (`orphaned_model_effect`). No model-effect request can escape
   every turn's verdict.
...
- `evidence/fail-closed-proofs.json` — deterministic zero-turn proofs
  ...
  target/conversation-id redaction, timeout fail-closed state machine, and
  exhaustive per-turn model-effect scope incl. pre-turn windows),
